...
export default function Hud({ state, score=0, onTimeOut, accuracyFlag=false, accuracy=0}: { state: "playing" | "afk" | "count-in", score:number,accuracy?:number, accuracyFlag:boolean, onTimeOut?: () => void }) {
    ...
    return (
        <div className="absolute top-20 left-0 w-full flex justify-center p-4 z-50">
            <div className="flex flex-col items-center text-lg font-bold orbitron-regular bg-[wheat] bg-opacity-50 text-black px-4 py-2 rounded">
                ...
                <Score score={score} />
                {accuracyFlag && <Accuracy accuracy={accuracy}></Accuracy>}
            </div>
        </div>
    );
}